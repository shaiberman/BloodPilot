...
def get_trivia():
    questions = pd.read_csv(_thisDir + "/Kanga_TriviaList_ADHD.csv", usecols=['QuestionNum', 'Question', 'AnswerUse'])
...
def get_wtp_trivia(subjectId):
    ...
    # old_questions1 = get_questions_used_in_wtw_task(subjectId)
    old_questions = get_questions_used_in_rating_task(subjectId)
    # old_questions = old_questions1 + old_questions2
...
def get_unused_questions(subjectId):
    ...
    #old_questions1 = get_questions_used_in_wtw_task(subjectId)
    old_questions1 = get_questions_used_in_wtp_task(subjectId)
    old_questions2 = get_questions_used_in_rating_task(subjectId)
    old_questions = old_questions1 + old_questions2 #+ old_questions3
    questions = questions.loc[~questions['Question'].isin(old_questions)]

    unused_questions = questions['Question'].values

    random.shuffle(unused_questions)
    """
    all_questions = get_trivia_questions()
    old_questions1 = get_questions_used_in_wtw_task(subjectId)
    old_questions2 = get_questions_used_in_wtp_task(subjectId)
    old_questions3 = get_questions_used_in_rating_task(subjectId)
    old_questions = old_questions1 + old_questions2 + old_questions3
    ...
    j = 0
    nQuestions = 100
    while i < nQuestions:  # there are hundreds of questions, so should not be an infinite loop
        q = all_questions[j]
